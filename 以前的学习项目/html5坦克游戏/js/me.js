...
function canvasApp() {
	...
	var tank=new Image();
	tank.addEventListener('load', start, false);         //addEventListener() 方法用于向指定元素添加事件句柄。
	...
	context.strokeStyle = "#000000";  //设置或返回用于笔触的颜色、渐变或模式
	context.strokeRect(8,8, theCanvas.width-16, theCanvas.height-16);  //绘制矩形（无填充）
	//画我方tank和我方炮弹
	function drawtank() {
		if(gameover){
			context.save();
			...
			context.fillText("游戏结束", 150, 150);  //在画布上绘制“被填充的”文本
